...
class Solution:
    ...
    def findMode(self, root: TreeNode) -> List[int]:
        self.dfs(root)
        values = sorted(self.cnt.items(), key=lambda x:x[1])
        max_cnt = values[-1][1]
        res = []
...
def test():
    func = lambda x: Solution().findMode(x)
    ...
    print(func(root6))
    root6 = create_binary_tree_from_level_order_list([1,null,2,2])
    print(func(root6))
    root6 = create_binary_tree_from_level_order_list([0])
    print(func(root6))
